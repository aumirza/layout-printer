...
import { MeasurementUnit, PageSize, LayoutPreset, SpaceOptimization } from '@/types/collage';
...
export function InitialSetupModal({ open, onClose, onApplySettings }: InitialSetupModalProps) {
  const [pageSizeIndex, setPageSizeIndex] = useState(0);
  const [layoutIndex, setLayoutIndex] = useState(0);
  const [spaceOptimization, setSpaceOptimization] = useState<SpaceOptimization>('loose');
  ...
  const handleApply = () => {
    ...
    localStorage.setItem('collagePreferences', JSON.stringify({
      pageSizeIndex,
      layoutIndex,
      spaceOptimization,
      selectedUnit
  ...
  return (
    <Dialog open={open} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle className="text-center text-xl">Setup Your Collage</DialogTitle>
        </DialogHeader>
        ...
        <div className="grid gap-4 py-4">
          <div className="flex justify-between items-center mb-2">
            ...
            <div className="flex items-center space-x-2">
              <Select value={selectedUnit} onValueChange={(value: MeasurementUnit) => setSelectedUnit(value)}>
          ...
          <div className="grid gap-2">
            ...
            <Select value={pageSizeIndex.toString()} onValueChange={(value) => setPageSizeIndex(parseInt(value))}>
              <SelectTrigger>
          ...
          <div className="grid gap-2">
            ...
            <Select value={layoutIndex.toString()} onValueChange={(value) => setLayoutIndex(parseInt(value))}>
              <SelectTrigger>
              ...
              <SelectContent>
                {layoutPresets.map((layout, index) => (
                  <SelectItem key={layout.id} value={index.toString()}>
                    {layout.label} ({formatDimension(layout.cellWidth)}×{formatDimension(layout.cellHeight)})
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>
          ...
          <div className="flex justify-between items-center">
            ...
            <div className="flex items-center space-x-2">
              ...
              <Switch 
                ...
                onCheckedChange={(checked) => setSpaceOptimization(checked ? 'tight' : 'loose')}
              />
          ...
          <div className="bg-muted/30 rounded-md p-3 mt-2">
            ...
            <p className="text-sm">This will create approximately <strong>{calculatedCells}</strong> photo cells.</p>
          </div>
        </div>
